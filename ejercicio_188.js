...
const searchNumbDivi = (numbers,numDv) =>{
    ...
    return numbers.map((item,index)=>{return{
        numeroDivisible: item.find((item)=>item%numDv === 0),
        indiceArrayExterno: index, 
        indiceArrayInterno: item.findIndex((item,index)=>item%numDv===0)
    }}).find((item)=>item.numeroDivisible%numDv === 0)

}
console.log(searchNumbDivi([[2, 2, 4], [7, 8, 9], [10, 12, 15]], 3));
